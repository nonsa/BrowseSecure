...
var handleMessage=function(a){if(a.message=="siteOnOff")disabledHere=a.disabled;else if(a.message=="requestBlocked"){TRUE_BLOCK=!0;var b=a.rule.name,c=a.url;c[0]=="/"&&(c=window.location.protocol+c);var d=urlToNodeMap[c];if(d&&d.length>0){if(a.isBlocked)for(a=0;a<d.length;a++)urlBlocked(d[a],b)}else a.isBlocked&&(urlAlreadyBlocked[c]={rule:b,type:a.type});d&&delete urlToNodeMap[c]}else a.message=="toggleTracker"?toggleTrackerHandler(a):a.message=="addIframe"&&window.top==window&&DNTP.addNotificationIframe(a.link,
document)},pageActions={getVersion:function(a){!topDomain||!topDomain.match(DNTP.abineDomainsRegex)?a(""):a(VERSION+" "+bridge.browser_name+" "+ORIGIN+" "+REVISION)},getDailyTotals:function(a,b){!topDomain||!topDomain.match(DNTP.abineDomainsRegex)?b(""):bridge.extension.sendRequest({message:"getDailyTotals",year:a},function(a){b(a)})},getBadTrackerTotals:function(a){if(!topDomain||!topDomain.match(DNTP.abineDomainsRegex)||DNTP.DISABLE_BAD_TRACKERS)return"";bridge.extension.sendRequest({message:"getBadTrackerTotals"},
function(b){a(b)})},showSettings:function(a){if(topDomain&&topDomain.match(DNTP.abineDomainsRegex)){var b=document.getElementById("noAddon");if(b)b.style.display="none";if(b=document.getElementById("addonPresent"))b.style.display="";bridge.showSettings()}a("")},changeLocale:function(a,b){window.location.host.match(DNTP.abineDomainsRegex)&&bridge.extension.sendRequest({message:"changeLocale",locale:a},function(){});b("")}};
function handlePageEvent(a){var b=a.target.getAttribute("action");if(b in pageActions){var c=a.target.getAttribute("param"),c=c?JSON.parse(c):[];c.push(function(b){a.target.innerHTML=b;DNTP.fireEvent(a.target,"click")});pageActions[b].apply(pageActions,c)}}
